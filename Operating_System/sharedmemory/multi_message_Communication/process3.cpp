...
int main() {
    ...
    semctl(semid, SEM_MUTEX34, SETVAL, 1);
    ...
    for (int i = 0; i < 5; i++) {
        semop(semid, &lock, 1);
        snprintf(shm->buffer34, MAX_MSG_SIZE, "Message %d from Process3", i + 1);
        shm->flag34 = 1;
        semop(semid, &unlock, 1);

        cout << "[Process3] Sent: " << shm->buffer34 << endl;
        sleep(1);
    }

    semop(semid, &lock, 1);
    snprintf(shm->buffer34, MAX_MSG_SIZE, "END");
    shm->flag34 = 1;
    semop(semid, &unlock, 1);
